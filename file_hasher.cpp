...
// simple sha-256 implementation adapted from public domain code
...
namespace {
    ...
    uint32 ROTRIGHT (uint32 a, uint32 b) {
        return (((a) >> (b)) | ((a) << (32-(b))));
    }
...
std::string compute_sha256(const std::string& file_path) {
    ...
    if (!file.is_open()) {
        return ""; // fle not found or can't open
    }
    ...
    while (file.good()) {
        ...
        if (bytesRead > 0) {
            sha256_update(&ctx, reinterpret_cast<uint8_t*>(buffer.data()), bytesRead);
        }
    }
